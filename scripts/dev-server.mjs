...
http.createServer(async(req,res)=>{
  try{
    ...
    for await(const chunk of req){body+=chunk;if(Buffer.byteLength(body)>10000){res.writeHead(413);return res.end();}}
    ...
    if(['/api/dados','/api/relatorios-acesso'].includes(url.pathname)){
      const handler=url.pathname==='/api/dados'?dataHandler:authHandler;
      const response=await handler({httpMethod:req.method,headers:req.headers,queryStringParameters:Object.fromEntries(url.searchParams),body});res.writeHead(response.statusCode,response.headers);return res.end(response.body);
    }
