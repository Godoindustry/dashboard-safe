...
const state={data:null,report:null,type:'weekly',etag:'',timer:null,loading:false,aiBusy:false,photos:[]};
const MAX_PHOTOS=12,MAX_SIDE=1400;
// As fotos nunca saem deste navegador: sao reduzidas em memoria e gravadas direto no PDF.
  ...
  return new Promise((resolve,reject)=>{
    ...
    reader.onload=()=>{
      ...
      img.onload=()=>{
        ...
        resolve({id:`${Date.now()}-${Math.random().toString(36).slice(2,8)}`,name:file.name,caption:'',width:w,height:h,dataUrl:canvas.toDataURL('image/jpeg',0.72)});
      };
...
function renderPhotos(){
  ...
  $('report-photo-count').textContent=list.length?`${list.length} de ${MAX_PHOTOS} foto(s) anexada(s). Entram no PDF na ordem abaixo.`:'Nenhuma foto anexada. O PDF é gerado normalmente sem elas.';
  $('report-photo-list').innerHTML=list.map((p,i)=>`<figure class="photo-item"><img src="${p.dataUrl}" alt="Foto ${i+1}: ${e(p.name)}"><label class="photo-caption">Legenda<input type="text" maxlength="160" data-photo-caption="${p.id}" value="${e(p.caption)}" placeholder="Onde foi, o que mostra"></label><button class="button ghost" type="button" data-photo-remove="${p.id}">Remover</button></figure>`).join('');
  $('report-photo-preview').innerHTML=list.length?list.map((p,i)=>`<figure><img src="${p.dataUrl}" alt="Foto ${i+1}"><figcaption>Foto ${i+1}${p.caption?` — ${e(p.caption)}`:''}</figcaption></figure>`).join(''):'<p class="report-empty">Nenhuma foto anexada a este relatório.</p>';
...
async function load(){
  ...
  try{
    ...
    if(!state.report)createPreview();else if(changed)status('Há dados novos na planilha. Clique em Gerar prévia para atualizá-la. Seu texto foi preservado.');
...
document.querySelectorAll('[data-report-type]').forEach(b=>b.addEventListener('click',()=>{state.type=b.dataset.reportType;document.querySelectorAll('[data-report-type]').forEach(x=>x.classList.toggle('active',x===b));$('week-control').hidden=state.type!=='weekly';$('month-control').hidden=state.type!=='monthly';createPreview();}));
for(const id of ['report-week','report-month','report-sector','report-filter-status','report-priority','report-search'])$(id).addEventListener('change',()=>createPreview());
...
$('print-button').addEventListener('click',async()=>{
  ...
  try{state.report.photos=state.photos;const {downloadReport}=await import('./pdf.bundle.js');downloadReport(state.report);status(`PDF gerado com todos os registros da prévia, as observações revisadas${state.photos.length?` e ${state.photos.length} foto(s)`:''}.`);}
