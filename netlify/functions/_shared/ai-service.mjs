...
export async function handleAI(request, { store, snapshot = getSnapshot, upstream = fetch } = {}) {
  ...
  if (!key) return reply(503, { error: 'Ative a integração Groq nas variáveis do Netlify. Nenhuma chave é solicitada ao visitante.' });
  let budget;
  try {
